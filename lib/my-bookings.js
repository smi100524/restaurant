...
(function () {
  ...
  style.textContent = [
    ...
    ".mb-actions{display:flex;gap:8px;margin-top:8px;padding-top:8px;border-top:1px dashed #e5ddc9}",
    ".mb-btn{border:1px solid #5f7f79;background:#fff;color:#2f5d50;border-radius:6px;padding:6px 12px;font-size:12px;cursor:pointer;font-family:inherit}",
    ".mb-btn:hover{background:#eef3ef}"
  ].join("\n");
  ...
  async function query() {
    ...
    try {
      ...
      if (Array.isArray(o)) o.forEach(function (x) { entries.push({ kind: "order", ts: x.ts || 0, date: x.book_date || "", time: x.book_time || "", room: x.room || "", people: x.people || 1, note: x.note || "", no: x.no || "", items: Array.isArray(x.items) ? x.items : [], total: x.total || 0, name: x.name || "", phone: x.phone || "", dept: x.dept || "", by: x.booker || "" }); });
    } catch (e) {}
    ...
    list.innerHTML = entries.map(function (e, idx) {
      ...
      if (e.kind === "order" && e.items && e.items.length) {
        html += '<div class="mb-items">' + e.items.map(function (i) { return '<div class="mb-it"><span>' + esc(i.name) + (i.option ? "（" + esc(i.option) + "）" : "") + " × " + i.qty + "</span><b>¥" + (i.price * i.qty) + "</b></div>"; }).join("") + "</div>" +
          '<div class="mb-total">合计 ¥' + e.total + "</div>";
      }
      if (e.kind === "order") {
        html += '<div class="mb-actions">' +
          '<button class="mb-btn" data-mb="print" data-idx="' + idx + '" type="button">🖨 打印菜单</button>' +
          '<button class="mb-btn" data-mb="invite" data-idx="' + idx + '" type="button">✉ 邀请函</button>' +
          "</div>";
      }
      html += "</div>";
      return html;
  ...
  function toOrder(e) {
    return {
      no: e.no, name: e.name || "客人", phone: e.phone || "", dept: e.dept || "",
      by: e.by || "", date: e.date, time: e.time, people: e.people || 1,
      room: e.room || "西式包间", note: e.note || "", items: e.items || [], total: e.total || 0
    };
  }
  modal.addEventListener("click", function (e) {
    if (e.target === modal) close();
    var mb = e.target.closest("[data-mb]");
    if (mb) {
      var ent = lastEntries[+mb.dataset.idx];
      if (!ent) return;
      try {
        if (mb.dataset.mb === "print") { toast("正在生成包间菜单…"); buildBaojianXlsxFile(toOrder(ent)); }
        else if (mb.dataset.mb === "invite") { close(); drawInvite(toOrder(ent)); }
      } catch (err) { try { toast("操作失败：" + err.message); } catch (e2) {} }
      return;
    }
