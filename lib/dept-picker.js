...
(function () {
  ...
  style.textContent = [
    ...
    ".dept-caret{position:absolute;right:6px;top:50%;transform:translateY(-50%);width:26px;height:26px;border:1px solid #ddd2ba;border-radius:6px;background:#fffdf7;color:#8a674d;font-size:11px;cursor:pointer;line-height:1;display:flex;align-items:center;justify-content:center;padding:0}",
    ".dept-caret:hover{background:#efe7d4}",
    ".dept-caret.open{background:#2f5d50;color:#fffdf7;border-color:#2f5d50}",
  ...
  function init() {
    ...
    Array.prototype.forEach.call(inputs, function (inp) {
      ...
      caret.type = "button";
      caret.className = "dept-caret";
      caret.setAttribute("aria-label", "选择部门");
      caret.innerHTML = "▾";
      ...
      function hide() { panel.style.display = "none"; caret.classList.remove("open"); active = -1; }
      function highlight() {
      ...
      document.addEventListener("click", function (e) { if (e.target && e.target.closest && !e.target.closest(".dept-panel") && !e.target.closest(".dept-caret") && e.target !== inp) hide(); });
      document.addEventListener("touchstart", function (e) { if (e.target && e.target.closest && !e.target.closest(".dept-panel") && !e.target.closest(".dept-caret") && e.target !== inp) hide(); });
    });
  }
